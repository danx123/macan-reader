...
# --- CLASS BARU UNTUK MENANGANI FILEOPENEVENT ---
class MacanApplication(QApplication):
    """QApplication kustom untuk menangani event FileOpen saat app sudah berjalan."""
    fileOpened = Signal(str) # Sinyal untuk kirim path file

    def __init__(self, argv):
        super().__init__(argv)
    
    def event(self, event):
        """Override event handler untuk menangkap FileOpen."""
        if event.type() == QEvent.Type.FileOpen:
            file_path = event.file()
            if file_path and file_path.lower().endswith('.pdf'):
                # Kirim sinyal ke window utama bahwa ada file baru
                self.fileOpened.emit(file_path)
                return True
        
        # Proses event lain seperti biasa
...
class MacanReader(QMainWindow):
    def __init__(self, file_to_open=None):
        ...
        session_restored = self.restore_session()

        # SELALU buka file_to_open jika ada (baik dari startup atau drag-drop)
        # Ini akan menambahkannya sebagai tab baru di atas session
        if file_to_open:
            self.open_pdf_in_new_tab(file_to_open)

        # Hanya tambahkan placeholder jika TIDAK ada session
        # DAN TIDAK ada file yang dibuka pas startup
        if not session_restored and not file_to_open:
...
if __name__ == "__main__":
    # Gunakan QApplication kustom kita dari Langkah 2
    app = MacanApplication(sys.argv)
    ...
    # file_to_open diteruskan ke init (sudah kita perbaiki di Langkah 3)
    window = MacanReader(file_to_open)
    
    # === INI BAGIAN PENTING ===
    # Hubungkan sinyal 'fileOpened' dari 'telinga' (app)
    # ke fungsi 'open_pdf_in_new_tab' (window)
    app.fileOpened.connect(window.open_pdf_in_new_tab)
